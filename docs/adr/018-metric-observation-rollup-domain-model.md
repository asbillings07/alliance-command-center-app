...
**Cache invalidation is a single, uniform contract across every observation-changing write, not something correction/void needs and recording does not.** `members/page.tsx` reads `MemberMetricEntry` directly, and the alliance dashboard displays `getAllianceSetupStatus`'s entry-sensitive completion signal via `SetupProgressCard` — both are affected identically whether a value arrived by manual recording, import, correction, void, or reactivation. `revalidateAllianceData` domains are `members`, `dashboard`, `setup`, `evaluation-results`, and `reports` for **all five** observation-changing write paths: manual recording (`record/action.ts`), single-period import, multi-period import (`multiPeriodAction.ts`), and the new correction/void/reactivation mutation. This is the same five-domain set the single-period [import action](<../../app/alliances/[allianceId]/periods/[periodId]/import/action.ts>) already uses today; `recordMemberMetrics`'s current three-domain set (`setup`, `evaluation-results`, `reports`) is the outlier and must be widened to match, not preserved as a narrower "recording-only" footprint. Next.js's current behavior of also refreshing already-visited pages on Server Function revalidation does not substitute for this — that behavior is explicitly documented as temporary, so correctness must rest on which paths are intentionally invalidated ([Next.js `revalidatePath` docs](https://nextjs.org/docs/app/api-reference/functions/revalidatePath)).
...
- Three write paths (`record/action.ts`, `periods/[periodId]/import/action.ts`, `multiPeriodAction.ts`) need an `observedOn` input and grain-aware validation. `record/action.ts` additionally needs its `revalidateAllianceData` call widened from three domains to the uniform five-domain set (`members`, `dashboard`, `setup`, `evaluation-results`, `reports`) per §2 — this is a pre-existing gap the ADR surfaces, not a new one it introduces.
...
- **The `PERIOD_VALUE`/`LATEST` defaults are migration-only and must be dropped from `Metric.observationGrain`, `Metric.memberPeriodRollup`, and `MemberMetricEntry.observationGrain` once every writer explicitly supplies them** — but *not* in the same migration that adds the defaults. This repository's `prisma migrate deploy` runs before the new application code takes over ([ADR-011](011-continuous-delivery.md)), so an old, still-running instance would fail on its first insert the instant the default disappeared. Dropping the defaults is therefore a separate, later migration, gated on every writer already being deployed and baked in production — the exact sequencing (expand → deploy explicit writers → bake → contract) is the [#287 database design](../database-design/287-metric-observations-and-member-period-rollups.md)'s §1, which this ADR defers to rather than duplicates. Leaving those three defaults in place indefinitely would let a newly created metric omit `observationGrain`/`memberPeriodRollup` entirely and silently become `PERIOD_VALUE + LATEST`, directly contradicting §1's requirement that a new metric select both explicitly — the database would quietly reintroduce the exact "guessed grain" failure mode §1 rules out; dropping them eventually, just not immediately, is what closes that gap safely. `MemberMetricEntry.status`'s `ACTIVE` default is the one exception and stays permanently: unlike grain/rollup, "a newly recorded entry is active unless explicitly voided" is the correct default forever, not just during backfill.
...
- The correction/void mutation fails closed for a user lacking `IMPORT_METRICS`, and for a slot whose period/metric/member does not belong to the acting alliance; a successful void/correction, and a successful manual recording, each invalidate the same `members`, `dashboard`, `setup`, `evaluation-results`, and `reports` cache domains.
