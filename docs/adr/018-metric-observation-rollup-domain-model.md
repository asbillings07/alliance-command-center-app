...
- **Cache invalidation**: `revalidateAllianceData` domains must be a superset of `recordMemberMetrics`'s own (`setup`, `evaluation-results`, `reports`), adding `members` — a void or correction changes the per-member daily-ledger drill-down and the members grid, a read surface recording alone does not currently need to invalidate.
...
- The correction/void mutation fails closed for a user lacking `IMPORT_METRICS`, and for a slot whose period/metric/member does not belong to the acting alliance; a successful void/correction invalidates the `members`, `setup`, `evaluation-results`, and `reports` cache domains.
