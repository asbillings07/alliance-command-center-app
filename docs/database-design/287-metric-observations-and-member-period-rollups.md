...
| `metricPeriodAllianceLockOrdering.integration.test.ts` | A daily-observation insert (holding `MetricPeriod` `FOR SHARE` via 4c, about to touch `Alliance` via `touchAllianceSetupActivity`) runs concurrently with a boundary edit on a *different* period (holding that period's `MetricPeriod` lock, about to touch the same `Alliance` row) — both must commit without deadlocking, proving the `MetricPeriod`-before-`Alliance` ordering holds across writers, not just within one |
