...
This remains deadlock-safe, but for a different reason than "only one row is locked": `FOR SHARE` locks never conflict with other `FOR SHARE` locks, on the same row or different rows, regardless of acquisition order. Two concurrent multi-period imports — even ones that touch the same two periods in opposite orders — can each acquire `FOR SHARE` on both periods without ever blocking each other, because neither is ever waiting on a lock **mode** the other holds. A deadlock cycle needs two transactions each holding a lock the other is waiting for; the only conflicting (non-`FOR SHARE`) lock in this system is the single-row `UPDATE` a boundary edit ([`periods/action.ts`](<../../app/alliances/[allianceId]/periods/action.ts>)) takes, and that write path never acquires a second lock while holding the first, so it can never sit on one side of a two-resource wait-for cycle. The properties that make this safe — every conflicting acquirer takes at most one lock, and every multi-lock acquirer (multi-period import) only ever takes the mutually-compatible `FOR SHARE` mode — are exhaustive over §8's writer inventory today; a future writer that takes a **conflicting** lock on more than one `MetricPeriod` row in a single transaction would need this analysis revisited.
...
| `metricResolutionCreateRequiresExplicitGrain.integration.test.ts` | §8's `metricResolution.ts` `create` disposition rejects a request missing `observationGrain`/`memberPeriodRollup` outright, and creates the metric with exactly the caller-supplied values when present — never a silent fallback |
...
The importer already has a real leader-confirmation surface for exactly this decision, so the fix is to use it rather than invent a new one: [`ColumnTranslationCard.tsx`](<../../app/src/components/spreadsheet/ColumnTranslationCard.tsx>) is where a leader explicitly picks **"Create new metric"** from a dropdown for an unmatched column (`canCreateMetrics`-gated, already requiring `CONFIGURE_METRICS`) before any import runs — this is "the import plan" that has to carry the confirmation. `ColumnTarget`'s `create` variant ([`importTranslation.ts:7`](<../../app/src/lib/importTranslation.ts#L7>)) and `ImportMetricTarget`'s `create` variant ([`metricResolution.ts:24`](<../../app/src/lib/metricResolution.ts#L24>)) must both grow `observationGrain`/`memberPeriodRollup` fields, populated by a small addition to that same card's already-existing "Create new metric" control (e.g. a grain radio group that appears once "create" is selected, with the rollup selector appearing only if `DAILY_OBSERVATION` is chosen) — not a new screen or flow. `resolveMetricTargets` must then use the caller-supplied values in its `tx.metric.upsert` call and reject (not default) a `create` disposition that omits them, closing the gap at the source rather than moving where the guess is hardcoded.

This is Implementation-phase work, not this design's deliverable (this design fixes the schema/migration contract, not the import UI), but it is a required, named part of Phase 1's writer inventory: `metricResolution.ts`'s writer cannot ship supplying an implicit default, unlike `createMetric` above, and the implementation PR that changes this writer must include the `ColumnTranslationCard` extension in the same vertical slice, not defer it.
