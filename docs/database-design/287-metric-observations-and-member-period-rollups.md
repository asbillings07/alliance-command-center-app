...
| `metricPeriodAllianceLockOrdering.integration.test.ts` | Two parts, both against the **same** `MetricPeriod` row and the **same** `Alliance` row (using different periods, as an earlier draft of this test did, would let the two transactions serialize purely on `Alliance` without ever contending on `MetricPeriod`, so a writer that reversed the order would still pass). **(a) Regression baseline:** the real `record/action.ts` insert and the real `periods/action.ts` boundary edit, run concurrently against the same period, both commit — proving today's code, which locks `MetricPeriod` before `Alliance` in both paths, cannot deadlock. **(b) Hazard canary, raw SQL:** two manually-sequenced sessions simulate the two orderings directly — one holds `MetricPeriod`'s lock and then requests `Alliance`'s, the other is deliberately driven `Alliance`-then-`MetricPeriod` (the order this design forbids) — and asserts Postgres's deadlock detector aborts one of them. (b) is what makes (a) a meaningful regression test rather than a trivial pass: it proves the ordering is actually load-bearing, so a future writer that silently reverses it would reproduce (b)'s failure in what should have been (a)'s success path |
...
- [`validateColumnTargets`](<../../app/src/lib/metricImport.ts#L155>) must reject a mapping outright — before anything is persisted — when the resolved target's grain is `DAILY_OBSERVATION` and `observedOn` is missing or not a well-formed calendar date, *and* when the grain is `PERIOD_VALUE` and `observedOn` is present (a stray date on a period-value column is rejected, not silently dropped, matching this function's existing all-or-nothing validation style). For a `create` target the grain is already on the wire (the previous finding's fix); for `existing`/`attach` targets the caller must resolve the already-configured metric's grain first — the same `LibraryMetric` extension that finding already requires (`observationGrain`/`memberPeriodRollup` added to the `prisma.metric.findMany` both action files already run) is what makes this lookup possible, not a second new field.
