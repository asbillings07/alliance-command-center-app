...
import type { AllianceSetupStatus } from "@/app/src/lib/allianceSetup";
import type { TargetPeriod } from "@/app/src/lib/periods/resolveTargetPeriod";
import { Card, Badge, SetupProgressCard } from "@/app/src/components";
import { Button } from "@/app/src/components/client";

export type LegacyDashboardProps = {
  allianceId: string;
  role: AllianceRole;
  permissions: PermissionSet;
  setupStatus: AllianceSetupStatus;
  activePeriod: TargetPeriod | null;
...
/**
 * The pre-#192 alliance dashboard, extracted byte-for-byte from `page.tsx`
 * so the `dashboard-workflow-groups` flag's disabled path is provably
 * unchanged (#332 AC: "Disabled production renders the existing dashboard
 * unchanged"). Deliberately does not import `dashboardWorkflowViewModel.ts`
 * or any other #192 code, so it can never be affected by changes there.
 */
