...
describe("RosterImportForm [component]", () => {
    ...
    it("renders Continue Setup when a valid returnTo is provided", async () => {
        (importMembers as ReturnType<typeof vi.fn>).mockResolvedValue({
            created: 1,
            restored: 0,
            skippedExisting: 0,
        ...
        const returnTo = `/alliances/${allianceId}/setup/import`;
        ...
        const continueLink = container.querySelector(`a[href="${returnTo}"]`);
        expect(continueLink).not.toBeNull();
        expect(continueLink?.textContent).toContain("Continue Setup");
    });

    it("does not render Continue Setup without returnTo", async () => {
        (importMembers as ReturnType<typeof vi.fn>).mockResolvedValue({
            created: 1,
            restored: 0,
            skippedExisting: 0,
